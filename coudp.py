...
class server_socket: # 서버쪽에서 클라이언트의 연결 요청을 받기 위해 생성하는 소켓
    ...
    def listen(self, num): # server socket이 최대 몇 개의 연결을 받아들일 수 있는지를 설정
        self.max_conn = num.self.set_state(state.READY)
    ...
    def chk_msg(self, msg, addr): # 연결을 기다리는 상황에서 클라이언트로부터 전달받은 메세지를 처리, 연결이 이루어졌으면 1을 리턴
        if self.state == state.WAIT_FOR_SYN:
            if msg.decode() == str(SYN_HDR):
                ...
                if self.max_conn > self.num_conn:
                    if (addr[0], addr[1], self.src_ip, self.src_port) in self.established_connections:
                        return 0
                    else:
                        self.set_state(self,WAIT_FOR_ACK)
                        self.sock.send(self, "SYNACK")
            ...
            if msg.decode() == str(ACK_HDR):
                ...
                if (addr[0], addr[1], self.src_ip, self.src_port) in self.established_connections:
                    ...
                    virtual_sock = virtual_socket(self,socket.SOCK_DGRAM )
                    established_connections.append(  ((addr[0], addr[1], self.src_ip, self.src_port), virtual_sock ))
    ...
    def recv(self, size): # server socket이 대표로 UDP 메세지를 수신
        ...
        list1 = list(established_connections)
        num = list1.index( addr[0], addr[1], self.src_ip, self.src_port )
        list1[num][1].buf(msg)
