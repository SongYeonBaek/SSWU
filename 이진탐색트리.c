...
//자식이 하나인 노드의 수 
int get_onenode_count(TreeNode *node) {
	if (node == NULL) return 0;
	if (node->left == NULL && node->right == NULL) return 0;
	else if (node->left != NULL && node->right != NULL) return get_onenode_count(node->left) + get_onenode_count(node->right);
	else
		return 1+ get_onenode_count(node->left) + get_onenode_count(node->right);
...
int main(void)
{
	...
	printf("자식노드의 개수가 1인 노드의 개수 = %d\n", get_onenode_count(root));
